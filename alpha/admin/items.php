<?php
	...
	$update = false;
	$successUpdateMsg = "";
	$ARRAY = array('type_id', 'category_id', 'item_meal', 'item_name', 'item_price' , 'item_in_stock', 'item_details', 'item_subitem_price' ,'item_status');
	foreach($ARRAY as $v) {
		$ARRAYTEMP[$v] = '';
	}
											...
											<div class="formEl_b">
												<form id="validation" method="post" action="items" enctype="multipart/form-data">
													<fieldset>
														...
														</div>														<div class="section last">															<label>Is Meal?</label>															<div>																<select name="item_meal" id="item_meal">																	<option value="0">No</option>																	<option value="1" <?php echo ($ARRAYTEMP['item_meal'] == 1) ? 'selected' : '' ?>>Yes</option>																</select>															</div>														</div>
														<div class="section last">
										...
										<div class="load_page">
											<form class="tableName toolbar">
												...
												<script type="text/javascript">
													var dataSend = [{'item_id': 'Item ID','type_name':'Type Name', 'category_name': 'Category Name' ,'item_name':'Item Name', 'item_price':'Price' , 'item_in_stock' : 'Stock' , 'item_status':'Status'}];
													$.ajax({
														  type: "POST",
