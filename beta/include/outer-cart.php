			...
			if(isset($_SESSION['CART'])) {
				if($_SESSION['CART']['TOTAL'] > 0) {
					...
					foreach($_SESSION['CART'] as $key => $value) {
						if($key != 'TOTAL') {
							$str .= '<li>';
							$str .= '<div><span class="qty">'.$value['QTY'].' x </span>   <span class="name">'.$value['NAME'].'</span><a class="del fl-right" href="javascript:;" rel="id'.$key.'" ><img src="images/button-minus.gif" alt="" /></a><sapn class="price b fl-right"> &pound; '.number_format($value['TOTAL'],2).'</sapn></div><div class="clr"></div>';
							$str .= '</li>';
						}
					}
					$str .= '</ul>';
					...
					echo '<hr class="hr" /><div class="button-cart"><button type="button" class="btn Checkout-Button">Check Out</button></div>';
