...
$(function() {
	...
	$("input.datepicker_000").datepicker({
		autoSize: true,
		appendText: '(yy-mm-dd)',
		dateFormat: 'yy-mm-dd'
	});
   ...
   $('.timepicker').timepicker({});
	...
	// mutiupload
	  $('.uploadFile').live('focusout',function(){
		//alert('file');
...
/*	$('.main_menu li ul li a').live({
	...
	$(".Delete").live('click',function() {
		  ...
		  var name = $(this).attr("name");
		  var data ='id='+id;
		  Delete(data,name,row,0,dataSet, id, tableName, colName);
...
function Delete(data,name,row,type,dataSet, delId, tableName, colName){
		...
		$.confirm({
		'title': '_DELETE DIALOG BOX','message': " <strong>YOU WANT TO DELETE </strong><br /><font color=red></font> ",'buttons': {'Yes': {'class': 'special',
		'action': function(){
					loading('Checking');

					$.ajax({
						  type: "GET",
						  url: 'include/confirmDelete.php',
						  data: { table : tableName , value : delId, column : colName},
...
$(document).ready(function(){
	...
	$('.chat-wrap textarea.text').keypress(function(event){
		var keycode = (event.keyCode ? event.keyCode : event.which);
		if(keycode == '13'){
			if(!FIRST_TIME_CHAT_START) {
				Get();
			}
		}
	});

	$('.chat-room .btn').live('click' ,function(){
		if(!FIRST_TIME_CHAT_START) {
			Get();
		}
	});
...
function Get() {
	var Text = getText($('.chat-wrap textarea.text').val());
	if(Text != false) {
		Put(Text);
		doAjax(Text);
	}
...
function doAjax(Text){
	...
	g = $.ajax({
			...
			data: { text : Text , type : 's', CID : CID},
			success: function(data) {
				$('.chat-room .chat ul').empty().html(data);
				var objDiv = document.getElementById("chatDIV");
				objDiv.scrollTop = objDiv.scrollHeight;
				setTimeout(doAjax(''), 15900);
			}
	});
}

function Put(text){
	$('.chat-room .chat ul').empty().html('<li class="to"><span class="name">Staff</span> : <span class="text">'+text+'</span></li>');
	text = "";
	$('.chat-wrap textarea.text').val('').empty();
	$('.chat-wrap textarea.text').blur();
	setTimeout(function() {$('.chat-wrap textarea.text').focus();},100);
	var objDiv = document.getElementById("chatDIV");
	objDiv.scrollTop = objDiv.scrollHeight;
}
function getText(text) {
	var str = text.replace(/(<([^>]+)>)/ig,"").replace(/\s+/g, " ");
	if(str != "" || str != null)
		return str;
	else
		return false;
...
/* function makeOnline() {
		$.ajax({
			type: "GET",
			url: '../include/makeOnline.php',
			data: { text : Text , type : 's'},
			success: function(data) {
				if(data == 'false'){
...
function ping(){
	var c = $.ajax({
			type: "GET",
			url: '../include/checkchat.php',
			data: { name : $('#MYCHATNAME').val(), ID : $('#MYCHATNAMEID').val()},
			success: function(data) {
				if(data == 'false'){
