<?php
	...
	class database
	{
        ...
        function connect_db()
        {
            $this->con = mysqli_connect($this->db_HOST,$this->db_USER,$this->db_PASS) or die(mysqli_error($this->con));
            $m = mysqli_select_db($this->con, $this->database) or die(mysqli_error($this->con));
            return($this->con);
        }
        ...
        function affecte_db()
        {
            $tmp = mysqli_affected_rows($this->con);
            return($tmp);
        }
        ...
        {
            $str = "";
            $get = mysqli_query($this->con, "SHOW COLUMNS FROM ". $table);
        ...
        // class.inc.css must be declared in document
        function get_table($query,$names='',$url='',$extrafields='') //names array of fieldnames | $url array 1. fieldindex 2. url
        {
            ...
            //datafields
            while($row_data = mysqli_fetch_row($sql))
            {
                if($extrafields!='')
                {
        ...
        {
            $sql = mysqli_query($this->con, str_replace(";"," ",$query)."LIMIT 1;");
            ...
            if($fieldnames==='')
            {
                while($data_row = $data_row = mysqli_fetch_row($sql))
                {
                    for($k=0;$k<=(mysql_num_fields($sql)-1);$k++)
                    {
                        unset($meta);
