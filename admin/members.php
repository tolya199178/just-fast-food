<?php
	...
	$update = false;
	$successUpdateMsg = "";
	$ARRAY = array('user_name','user_status','user_email');
	foreach($ARRAY as $v) {
		$ARRAYTEMP[$v] = '';
	}
	...
	if(isset($_POST['add'])) {
		if(isset($_POST['update'])) {
			...
			$query = "UPDATE  `user` SET `user_status` = '".$_POST['user_status']."'  WHERE `id` = '".$_POST['id']."'";
...
<head>
  ...
  <!--[if lte IE 8]><script language="javascript" type="text/javascript" src="components/flot/excanvas.min.js"></script><![endif]-->
        ...
        <script type="text/javascript" src="js/jquery.min.js?v2.1.3"></script>
							...
							<div id="uploadTab">
								...
								<div class="tab_container">
									...
									<div id="tab1" class="tab_content">
										<div class="load_page">
											<form class="tableName toolbar" method="post" action="members">
												<div class="section last">
													<label>Name</label>
													<div>
														<label for=""><?php echo $ARRAYTEMP['user_name']?></label>
													</div>
												</div>
												<div class="section last">
													<label>Email</label>
													<div>
														<label for="" style="text-transform: none;"><?php echo $ARRAYTEMP['user_email']?></label>
													</div>
												</div>
										...
										<div class="load_page">
											<form class="tableName toolbar">
												...
												<script type="text/javascript">
													var dataSend = [{'id':'User ID','user_name':'Name', 'user_email': 'Email Address' ,'user_phoneno':'Phone No', 'user_address':'Address' , 'user_post_code':'Post Code' , 'user_city' : 'City', 'user_date_added': 'Date Added'}];
													$.ajax({
														  type: "POST",
														  url: 'include/browse-table.php',
														  data: { col : dataSend, table : '`user`', status : 'user_status' , editURL : 'members?edit=true&id=', delColumn : 'id' , where : '' , actualTable : 'user'},
