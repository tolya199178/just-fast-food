...
// Monthly Revenue
$revenue = $obj->query_db("SELECT ROUND(SUM(order_total)) AS `value_sum` FROM `orders` WHERE MONTH(order_date_added) = MONTH(CURDATE()) AND YEAR(order_date_added) = YEAR(CURDATE()) ");
$monthlyRev = $obj->fetch_db_array($revenue);
if ($monthlyRev['value_sum'] == ''){
    $rev = 0;
} else {
...
$updates = $obj->query_db("SELECT COUNT(*) FROM `orders` WHERE `order_date_added` BETWEEN CONCAT(CURDATE(), ' ', '00:00:00') AND CONCAT(CURDATE(), ' ', '23:59:59') AND `order_status` = 'assign'");
$orderUps = $obj->fetch_db_array($updates);

// Current weekly profits.
$profits = $obj->query_db("SELECT * FROM `orders` WHERE MONTH(`order_date_added`) = MONTH(CURDATE()) AND YEAR(order_date_added) = YEAR(CURDATE())");
$value = 0;
while($pro = $obj->fetch_db_array($profits)) {
    $orderArray = json_decode($pro['order_details'], true);
    $orderTotalArray = json_decode($pro['order_total']);
    ...
    // Sum the Total Amount together.
    $sum +=  $orderTotalArray;

}
// The final price : We take out the total amount - the menu costs.
$final = $sum - $value;
    ...
    <div class="onecolumn">
        ...
        <div class="content">
            ...
            <div class="grid1 rightzero">
                ...
                <!-- // breaks -->
                <div class="shoutcutBox"> <span class="fa fa-gbp fa-2x" style="color: green"></span>  <strong><?php echo $rev;?></strong>  <em> Revenue This Month</em>
                </div>
                <div class="breaks"><span></span></div>

                <div class="shoutcutBox"><span class="fa fa-gbp fa-2x" style="color: green"></span> <strong><?php echo round($final, 2); ?></strong><em> Menus Profit This Month</em></div>
                <div class="breaks"><span></span></div>
